...
export const TabCustomRandom = ({ size = 'M', variant, label, disabled, value, ...props }) => {
    const tabRef = useRef(null);
    const [isSelected, setIsSelected] = useState(false);

    const handleTabRef = useCallback(node => {
        if (node !== null) {
            setIsSelected(node.classList.contains('Mui-selected'));
        }
        tabRef.current = node;
    }, []);
    ...
    if (variant === 'pill') {
        return <CustomTabPill {...props} ref={handleTabRef} size={size} label={labelItem} />;
    } else if (variant === 'fill') {
        return <CustomTabFill {...props} ref={handleTabRef} icon={<PersonIcon color={iconColor} />} iconPosition='start' label={labelItem} />;
    }
    return <CustomTabDefault {...props} ref={handleTabRef} icon={<PersonIcon color={iconColor} />} iconPosition='start' label={labelItem} />;
};
